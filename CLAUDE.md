...
**`README.md`（中文）与 `README.en.md`（英文）是同一份内容的两版，改一处就要改另一处**——两边顶部都有语言切换链接，只改中文的话英文版会慢慢变成另一个项目的介绍。中文是主版本，英文跟着它走。`docs/` 目前只有中文，英文 README 里那句「文档暂时只有中文」在译完之前不要删。

发布包里带的是 `README.md` + `README.en.md` + 整个 `docs/`（见 `.github/workflows/release.yml`）：README 里的链接全指向 docs，不一并打包的话下载下来的包里全是死链。
