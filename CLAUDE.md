...
19. **会话注记旁听**（`NoticeObserver`，`internal/plugin/noticeobs.go`）—— 注记落盘并送达 Web 界面之后，由 `Manager.NotifyNotice` 广播给启用插件；IM 通道据此把后台工作的说明（如提炼的记录）按各自的「推送后台通知」开关推给绑定用户（`imbot.Core.PushNotice`），推送本身不产生任何会话记录。三类不推：带可见域标签的（归属拿不准，宁可少推不泄漏）、IM 通道自己发的（给操作者看的旁注，推出去会暴露另一条通道）、路由不归本通道的；也刻意不做转投那套「自动接入唯一用户」。事件携带的是已截断的落盘文本，远端看到的与 Web 界面一字不差。
