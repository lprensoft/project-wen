...
**定时类状态记「上一次发生的时刻」，不记「还剩多久」。** 进程内的定时器只是执行手段，不是状态载体——它随进程消失，于是重启就把倒计时清零重算，重启比周期更频繁时那件事一次都不会发生（心跳曾经如此：只存了间隔，没存上次心跳时刻）。存了时刻之后，下一次由「时刻 + 周期」推算，重启天然延续；已经过期就补一次，但要给一个启动宽限期，别在服务刚起来的那一秒就动作。同理，带时效的缓存要连同取得时刻一起落盘（天气的观测），否则重启既产生数据空窗，又白打一次外部接口。`scheduler` 的 `LastRun`、`mood` 的 `Updated` 是这条的既有实现，新写定时插件时照着来。
