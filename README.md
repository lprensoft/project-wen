...
- **有状态的角色**：场景插件把当下的舞台与地点记成场景记忆并延续；身体感知按各部位被触碰的累计次数划分熟悉阶段，给出与之相称的反应尺度；心情随互动的演绎增减、随时间向平静回落，每轮注入并影响表达方式；天气感知按你所在的城市取现实天气，让「你那边在下雨」成为角色知道的事
...
对方那边在下雨还是刚放晴，是一条持续起作用的背景——它不构成话题，但影响关心的方向与说话的底色。本插件按你填写的城市定时取现实天气（数据源 [Open-Meteo](https://open-meteo.com/)，免费、无需密钥），把此刻的天气每轮注入一行。默认不启用，硬依赖 `roleplay`。

它注入的是**环境状态**而不是一个可调用的工具，这也是它归在「角色演绎」而不是「基础工具」的原因：通用对话里每轮塞一行天气只是噪声，真要问天气，模型自己用 `fetch_url` 抓就够了。

- **不填城市就什么都不注入**，包括那段讲天气怎么用的规则。位置是一条你自己完全知道、几乎不变的事实，让模型从记忆或场景里去推断，是把确定性问题换成概率问题——何况场景记忆里的地点可能是虚构的舞台。
- **天气是背景，不是话题**：注入的准则要求只在自然相关时提起（要出门、说到冷热、天气反常、被问到），不当开场白、不每轮都提。不写这一条，模型很容易每轮开头都来一句「外面还在下雨呢」。
- **与舞台设定冲突时以舞台为准**：`scene` 的舞台可能是个虚构的地方，而这里的天气说的是对方现实中所在的地方。两处不是同一个地方时，天气只用于对对方的关心，不写进场景描写。
- **后台刷新，注入只读缓存**：`TurnPrompt` 在每轮对话的同步路径上，在那里发网络请求就是让每一轮都替一次超时买单。
- **过期就不注入**：取不到新数据时，让角色不知道天气，好过把三小时前的天气当成此刻。因此过期时限不得小于刷新间隔——否则刚取到的也立刻算过期，等于永远不注入。未收录的天气代码回落到「天气不明」，注入的内容宁可含糊也不能是错的。
- **测试按钮**：设置页齿轮里有「测试这个城市」，读的是**还没保存**的输入，直接告诉你解析成了哪个地方、当时的天气是什么。地名写法五花八门、重名的地方也多，解析成了哪里只有查一次才知道。
- **状态行分开报**「没配城市」「取不到」「过期了」：这三种情况在对话里看起来都一样——角色都是不提天气。

| 配置项 | 默认 | 说明 |
|---|---|---|
| `location` | 空 | 我所在的城市，如「杭州」「上海」「Hangzhou」。留空则不查询、也不注入任何天气 |
