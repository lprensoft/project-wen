...
- **插件化架构**：精简核心（agent 循环 / session / server / llm）+ 插件扩展。工具能力全部由插件提供，插件可运行时开关（Web UI 设置页或 API），并可选择性向系统提示词注入片段、订阅会话生命周期事件。内置四个系统插件：`read_file`、`exec_command`、`web_fetch`、`memory`
- **长期记忆**：跨会话的持久事实库，索引随每次对话自动注入，模型可自行保存与读取；历史被压缩前先原样归档（见下）
...
- **存储**：`<配置目录>/plugins/memory/memories/<标题>.md`，一条记忆一个文件，YAML frontmatter（标题 / 摘要 / 分类 / 时间）+ Markdown 正文，可直接用编辑器增删改；手工放进去的普通 Markdown 也能被索引到。整个 `plugins/` 目录已在 `.gitignore` 中。
...
- **压缩归档**：历史被摘要替换**之前**，完整原文（含思考过程与工具调用）会被归档到 `<配置目录>/plugins/memory/archives/`，并在摘要末尾留一行归档位置。归档只做文件写入，不额外发起模型调用。
...
internal/plugin/builtin/ 内置系统插件：readfile / execcmd / webfetch / memory
...
- `Lifecycle`（`OnCompact()`）——在会话历史被摘要替换**之前**收到通知，可借此归档；返回的注记会追加到摘要末尾，从而只落进该会话的历史。返回错误只记日志，不阻断压缩。

需要落盘的插件用 `InitContext.StateDir`（`<配置目录>/plugins/<插件名>/`，可能不存在需自行创建）。该字段为空表示当前没有可用的持久化位置，应在 `Init` 中返回错误拒绝启用，不要退化到写进程当前目录。
...
- MCP 工具
- 会话搜索插件：记忆的补充层，用于翻查当时的原话。前提是压缩归档（已完成），因为原始历史会被压缩物理删除；另需注意会话会跨天使用，`Meta` 只有创建时间，按天检索要看每条消息的 `ts`
- 记忆按工作目录分项目（`scope` 配置项）：当前是全局单库，用 `~/.wen/config.yaml` 在多个项目目录下启动时记忆会串味
