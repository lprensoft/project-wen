...
- **会主动开口**：心跳按间隔唤醒模型在最近活跃的会话上说话，下次开口的时机由角色自己在对话里定——聊到兴头上挪近，你说要去忙就往后推；定时任务可以在对话中创建（一次性 / 周期 / 每日），到点后台执行并把结果写回会话
- **远程使用**：QQ、微信、飞书、Lark、Telegram 五条通道，各自映射一套私聊会话，支持 `/new` `/status` `/compact` `/help` 与危险操作的远程确认；后台轮次产生的回复会主动推送到远端
- **插件化架构**：精简核心（agent 循环 / session / server / llm）+ 插件扩展，十九个内置插件按功能分组呈现，可在设置页随时开关与配置，改动即时生效。核心只认识通用机制——提示词片段注入、会话生命周期、上下文可见域、辅助模型调用、插件自行发起轮次、操作确认、轮次过程通知、状态行、界面操作入口、只给人看的会话注记、插件间的依赖与冲突——它不知道「记忆」「人格」「心情」这些具体功能的存在
...
- **节奏由角色自己定**（默认开）：模型手上有一个 `set_heartbeat_interval` 工具，聊到兴头上就把下次开口挪近，你说要去忙、要去睡就往后推，定完倒计时从此刻重算。它是在对话中顺手调用的，因此不产生任何额外的模型调用；理由也随调用一起写下来，落在日志里。
- **没人说话时自己放缓**：空闲衰减定时器每 15 分钟把间隔放缓一档（×1.5），直到最慢间隔——不必等模型想起来调。间隔一律取整到分钟，并收进你设的上下限。
- **关掉动态心跳**就是固定间隔，模型也改不动：此时调用那个工具会被拒绝，并告诉它去哪儿打开。
...
| `dynamic` | 开 | 允许模型自己定下次开口的时机，并在无人聊天时逐步放缓；关掉则固定按基础间隔 |
...
internal/plugin/builtin/ 内置系统插件（注册顺序即提示词拼接顺序）：
                         ...
                         roleplay / dualpersona / scene / weather / bodysense / mood /
