package sessionsearch

import (
	"context"
	"encoding/json"
	"fmt"
	"os"
	"path/filepath"
	"strings"
	"testing"
	"time"

	"wen/internal/llm"
	"wen/internal/plugin"
)

func sampleEvent() plugin.CompactEvent {
	return plugin.CompactEvent{
		SessionID: "20260817-103426-8cb8ea",
		Summary:   "这是摘要",
		History: []llm.Message{
			{Role: llm.RoleUser, Content: "用户说的话"},
			{Role: llm.RoleAssistant, Content: "助手的回答", Reasoning: "思考过程",
				ToolCalls: []llm.ToolCall{{ID: "c1", Name: "echo", Arguments: json.RawMessage(`{"text":"hi"}`)}}},
			{Role: llm.RoleTool, Content: "工具返回的内容"},
		},
	}
}

// newArchivePlugin 返回一个同时有会话目录与持久化目录的插件。
func newArchivePlugin(t *testing.T, cfg map[string]any) *Plugin {
	t.Helper()
	base := t.TempDir()
	sessions := filepath.Join(base, "sessions")
	if err := os.MkdirAll(sessions, 0o755); err != nil {
		t.Fatal(err)
	}
	p := New()
	err := p.Init(plugin.InitContext{
		SessionDir: sessions,
		StateDir:   filepath.Join(base, "state"),
	}, cfg)
	if err != nil {
		t.Fatal(err)
	}
	return p
}

func archiveNames(t *testing.T, p *Plugin) []string {
	t.Helper()
	des, err := os.ReadDir(p.snapshot().archiveDir)
	if err != nil {
		if os.IsNotExist(err) {
			return nil
		}
		t.Fatal(err)
	}
	var names []string
	for _, de := range des {
		names = append(names, de.Name())
	}
	return names
}

func TestOnCompactArchivesFullHistory(t *testing.T) {
	p := newArchivePlugin(t, nil)
	ev := sampleEvent()

	note, err := p.OnCompact(context.Background(), ev)
	if err != nil {
		t.Fatal(err)
	}
	names := archiveNames(t, p)
	if len(names) != 1 {
		t.Fatalf("应生成 1 个归档，实际 %v", names)
	}
	if !strings.Contains(note, names[0]) {
		t.Errorf("注记应指出归档文件名: %q（文件 %q）", note, names[0])
	}

	raw, err := os.ReadFile(filepath.Join(p.snapshot().archiveDir, names[0]))
	if err != nil {
		t.Fatal(err)
	}
	body := string(raw)
	// 归档的意义在于保住原文：每一部分都不能丢，也不做截断
	for _, want := range []string{
		ev.SessionID, "这是摘要", "用户说的话", "助手的回答", "思考过程", "echo", "工具返回的内容",
	} {
		if !strings.Contains(body, want) {
			t.Errorf("归档缺少 %q:\n%s", want, body)
		}
	}
	// 临时文件不应残留
	for _, n := range names {
		if strings.HasSuffix(n, ".tmp") {
			t.Errorf("残留临时文件: %s", n)
		}
	}
}

func TestOnCompactSkippedWhenDisabled(t *testing.T) {
	p := newArchivePlugin(t, map[string]any{"max_archives": 0})
	note, err := p.OnCompact(context.Background(), sampleEvent())
	if err != nil {
		t.Fatal(err)
	}
	if note != "" {
		t.Errorf("关闭归档时不应返回注记: %q", note)
	}
	if names := archiveNames(t, p); len(names) != 0 {
		t.Errorf("关闭归档时不应写文件: %v", names)
	}
}

func TestOnCompactSkipsEmptyHistory(t *testing.T) {
	p := newArchivePlugin(t, nil)
	note, err := p.OnCompact(context.Background(), plugin.CompactEvent{SessionID: "s", Summary: "空"})
	if err != nil || note != "" {
		t.Errorf("空历史应直接跳过: note=%q err=%v", note, err)
	}
}

func TestOnCompactWithoutStateDir(t *testing.T) {
	dir := filepath.Join(t.TempDir(), "sessions")
	os.MkdirAll(dir, 0o755)
	p := New()
	if err := p.Init(plugin.InitContext{SessionDir: dir}, nil); err != nil {
		t.Fatal(err)
	}
	note, err := p.OnCompact(context.Background(), sampleEvent())
	if err != nil || note != "" {
		t.Errorf("没有持久化目录时应跳过归档而不是报错: note=%q err=%v", note, err)
	}
}

func TestPruneArchivesKeepsNewest(t *testing.T) {
	dir := t.TempDir()
	base := time.Now().Add(-10 * time.Hour)
	for i := range 5 {
		name := filepath.Join(dir, fmt.Sprintf("会话%d-x.md", i))
		if err := os.WriteFile(name, []byte("x"), 0o644); err != nil {
			t.Fatal(err)
		}
		// 故意让文件名字典序与时间序相反，验证淘汰依据是时间而不是文件名
		mod := base.Add(time.Duration(5-i) * time.Hour)
		if err := os.Chtimes(name, mod, mod); err != nil {
			t.Fatal(err)
		}
	}
	os.WriteFile(filepath.Join(dir, "readme.txt"), []byte("keep"), 0o644)

	pruneArchives(dir, 2)

	des, _ := os.ReadDir(dir)
	var kept []string
	for _, de := range des {
		if strings.HasSuffix(de.Name(), ".md") {
			kept = append(kept, de.Name())
		}
	}
	if len(kept) != 2 {
		t.Fatalf("应保留 2 个归档，实际 %v", kept)
	}
	for _, n := range kept {
		if n != "会话0-x.md" && n != "会话1-x.md" {
			t.Errorf("保留了较旧的归档: %v", kept)
		}
	}
	if _, err := os.Stat(filepath.Join(dir, "readme.txt")); err != nil {
		t.Error("非归档文件不应被删除")
	}
}

func TestOnCompactPrunesToLimit(t *testing.T) {
	p := newArchivePlugin(t, map[string]any{"max_archives": 3})
	for i := range 6 {
		ev := sampleEvent()
		ev.SessionID = fmt.Sprintf("会话%d", i)
		if _, err := p.OnCompact(context.Background(), ev); err != nil {
			t.Fatal(err)
		}
	}
	if names := archiveNames(t, p); len(names) != 3 {
		t.Errorf("应只保留 3 个归档，实际 %d 个: %v", len(names), names)
	}
}

func TestSanitizeID(t *testing.T) {
	cases := map[string]string{
		"20260817-103426-8cb8ea": "20260817-103426-8cb8ea",
		"a/b\\c":                 "a-b-c",
		"..":                     "session",
		"":                       "session",
	}
	for in, want := range cases {
		if got := sanitizeID(in); got != want {
			t.Errorf("sanitizeID(%q) = %q, want %q", in, got, want)
		}
	}
}

// TestSearchCoversArchives 是把归档搬到本插件的理由：被压缩过的会话只剩摘要，
// 若检索不覆盖归档，最值得回查的那部分就成了盲区。
func TestSearchCoversArchives(t *testing.T) {
	p := newArchivePlugin(t, nil)
	ev := sampleEvent()
	ev.History = []llm.Message{{Role: llm.RoleUser, Content: "当时说过 pineapple 这个词"}}
	if _, err := p.OnCompact(context.Background(), ev); err != nil {
		t.Fatal(err)
	}

	got := run(t, &searchTool{p: p}, `{"keyword":"pineapple"}`)
	if !strings.Contains(got, "压缩归档") || !strings.Contains(got, "pineapple") {
		t.Errorf("检索应覆盖压缩归档:\n%s", got)
	}
	if !strings.Contains(got, "1 个压缩归档命中") {
		t.Errorf("应统计归档命中数:\n%s", got)
	}
}

func TestReadArchive(t *testing.T) {
	p := newArchivePlugin(t, nil)
	if _, err := p.OnCompact(context.Background(), sampleEvent()); err != nil {
		t.Fatal(err)
	}
	name := archiveNames(t, p)[0]
	tool := &archiveTool{p: p}

	// 不传文件名 = 列出全部
	list := run(t, tool, `{}`)
	if !strings.Contains(list, "共 1 份压缩归档") || !strings.Contains(list, name) {
		t.Errorf("列表结果:\n%s", list)
	}

	full := run(t, tool, fmt.Sprintf(`{"name":%q}`, name))
	if !strings.Contains(full, "用户说的话") || !strings.Contains(full, "思考过程") {
		t.Errorf("读取全文缺内容:\n%s", full)
	}

	// 关键词收窄
	only := run(t, tool, fmt.Sprintf(`{"name":%q,"keyword":"工具返回"}`, name))
	if !strings.Contains(only, "工具返回的内容") || strings.Contains(only, "用户说的话") {
		t.Errorf("关键词收窄失效:\n%s", only)
	}
	if out := run(t, tool, fmt.Sprintf(`{"name":%q,"keyword":"绝不存在"}`, name)); !strings.Contains(out, "没有包含") {
		t.Errorf("无命中应有说明:\n%s", out)
	}

	// 不存在的归档、以及试图跳出归档目录
	if _, err := tool.Execute(context.Background(), json.RawMessage(`{"name":"没有这个"}`)); err == nil {
		t.Error("不存在的归档应报错")
	}
	if _, err := tool.Execute(context.Background(),
		json.RawMessage(`{"name":"../../config.yaml"}`)); err == nil {
		t.Error("跳出归档目录的路径应被拒绝")
	}
}

func TestPluginImplementsLifecycle(t *testing.T) {
	var _ plugin.Lifecycle = New()

	base := t.TempDir()
	sessions := filepath.Join(base, "sessions")
	os.MkdirAll(sessions, 0o755)
	m := plugin.NewManager(
		plugin.InitContext{SessionDir: sessions},
		filepath.Join(base, "plugins.state.json"),
	)
	if err := m.Register(New(), plugin.PluginConfig{Enabled: true}); err != nil {
		t.Fatal(err)
	}
	notes := m.NotifyCompact(context.Background(), sampleEvent())
	if len(notes) != 1 || !strings.Contains(notes[0], "已归档") {
		t.Errorf("Manager 广播未收到归档注记: %v", notes)
	}
}
